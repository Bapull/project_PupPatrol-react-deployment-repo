...
const WantMBTIQCard = ({ question, setAnswer, questionVariable, setCurrentQuestionId, leftover }) => {
  ...
  useEffect(() => {
    ...
    setTransformOrigin({
      x: target.getBoundingClientRect().right - target.getBoundingClientRect().left,
      y:
        target.getBoundingClientRect().bottom +
        (target.getBoundingClientRect().top - target.getBoundingClientRect().bottom) * 0.1,
    });
  ...
  return (
    ...
    >
      ...
      >
        <span className="question" dangerouslySetInnerHTML={{ __html: question }}></span>{' '}
      </div>
    </div>
  );
};
